...
export function If({ name, children, }: { name: string, children: React.ReactNode }) {
    return <Field name={name}>{({ field }: FieldProps) => {
        return field.value ? children : null
    }}
    </Field>
}

export function IfNot({ name, children }: { name: string, children: React.ReactNode }) {
    return <Field name={name}>{({ field }: FieldProps) => {
        return !field.value ? children : null
    }}
    </Field>
...
export function IfValid({ children }: { children: React.ReactNode }) {
    return <Field name={name}>
        {({ field, form }: FieldProps<any>) => form.isValid && children}/>
    </Field>
}
...
export function IfError({ children }: { children: React.ReactNode }) {
    return <Field>
        {({ field, form }: FieldProps<any>) => (form.error || form.errors) && children}/>
    </Field>
}

export function IfField({ name, children, value }: { name: string, children: React.ReactNode, value?: any }) {
    return <Field>
        {({ field, form }: FieldProps<any>) => value === undefined ? (field.value) && children : field.value == value && children}/>
    </Field>
}
